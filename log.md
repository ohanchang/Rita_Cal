...
## 2026/08/05 - v1.5 AI 歷史趨勢分析與表格 UX 優化
- **InBody 歷史數據表格優化**：表格加入「凍結首欄 (Sticky Header)」與「滾動影」，大幅提升在手機小螢幕上橫向滑動查看數據時的辨識度。
- **AI 導覽標籤更名**：將「AI 飲食建議」重新命名為「AI 綜合分析與建議」。
- **GLP-1 歷史整合分析**：在體重趨勢區塊前，新增專屬的 GLP-1 用藥紀錄 (Mounjaro) 分析結果，AI 會主動比對用藥劑量、頻率與 InBody 數據變化（體重/體脂流失率）的生理關聯。

## 2026/08/04 - v1.4 Mounjaro (GLP-1) 互動式日曆追蹤
- **日曆追蹤介面**：將原先單純的列表升級為互動式日曆，使用者可直接點選日期紀錄施打劑量。
- **修復 Notion 同步錯誤**：修復資料庫欄位尾端空白字元 (Trailing space) 導致排序查詢 (Sort API) `400 Bad Request` 錯誤。

## 2026/08/04 - v1.3 Mounjaro (GLP-1) 藥物紀錄與 InBody 數據連動
- **新增 GLP-1 藥物紀錄功能**：在 InBody 追蹤頁面加入快速藥物紀錄選單（2.5mg, 5.0mg, 7.5mg），並自動同步至 Notion Database。
- **AI 提示詞強化**：在解析 InBody 數據時，Gemini Prompt 將自動注入近 5 次 Mounjaro 用藥紀錄，強制要求 AI 排除瘦體重 (Lean Body Mass) 流失狀況，以防止 GLP-1 減重過程中的肌肉流失。
- **修正 InBody 備註讀取錯誤**：調整 AI Prompt，使其在辨識與解析數據時，優先採納使用者提供的備註說明。

## 🚀 v1.2 InBody AI 輔助文字說明與 UI 版本標記功能（2026-07-26）
