...
function buildDietAnalysisPrompt(
  ...
  return `你是一位專業營養師與代謝體態管理顧問。以下是使用者近期的 InBody 身體組成歷史數據、${periodLabel}的飲食紀錄，以及最近的 GLP-1 (Mounjaro 猛健樂) 施打紀錄。請根據這些資料做多維度交叉分析，給出專業的分析建議。

## 近期 InBody 歷史數據 (包含體重、骨骼肌、體脂肪率)
${inbodyJson}
...
## 分析要點
1. **GLP-1 綜合影響與歷史趨勢分析**：要求你將「InBody 變化趨勢」與「猛健樂施打日」放在同一個時間軸比較。觀察用藥頻率/劑量是否導致明顯的熱量缺口，以及伴隨的體重與「骨骼肌重」變化趨勢。
2. **肌肉流失警報**：若發現骨骼肌量連續下降比例明顯大於或等於體脂下降，必須在 \`mounjaroAssessment\` 中提出明確的肌肉流失警告，並建議增加蛋白質與阻力訓練。
3. **資料不足時的例外處理**：
   - 若 InBody 歷史資料不足 2 筆，請在 \`mounjaroAssessment\` 誠實告知「缺乏歷史數據，無法評估用藥後的長期體態趨勢」。
   - 若無猛健樂施打紀錄，請在 \`mounjaroAssessment\` 中填寫「近期無 GLP-1 施打紀錄」。
4. **熱量對比**：實際平均攝取 vs InBody 建議的每日攝取熱量，差距是否合理。
5. **蛋白質攝取**：根據體重和骨骼肌重，計算每公斤體重蛋白質攝取量，是否達到 1.2-1.6g/kg 建議範圍，特別是在 GLP-1 減重期間以預防肌少症。
6. **脂肪攝取**：體脂率偏高時，脂肪攝取比例是否仍然過高。
7. **膳食纖維**：是否達到每日 25g 的建議量。
8. **飲食均衡**：是否有營養素明顯不足或過量。
9. **具體可行建議**：建議需要具體且可操作（例如「每餐增加一份拳頭大小的蛋白質」而非「多吃蛋白質」）。
10. 如飲食紀錄不足 3 天，在 summary 中提醒數據量可能不足以做出精準分析。
